...
import { Clock, GripVertical, MapPin, Phone, UserRound } from 'lucide-react';
...
export const getDoctorSeqColumns = (
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="inline-flex h-7 min-w-7 items-center justify-center rounded-md border border-slate-200 bg-slate-50 px-2 text-xs font-bold text-slate-700">
          {row.original.sequence_no ?? row.index + 1}
        </span>
      ),
    },
    {
  ...
  columns.push(
    ...
    },
    {
      ...
      header: 'Avg Consultation Duration (mins)',
      size: 220,
      cell: ({ row }) => (
        <div className="flex items-center justify-center gap-1 font-bold text-slate-600">
          <Clock size={14} />
          {row.original.avg_consultation_duration ?? 0}m
        </div>
      ),
    },
    {
